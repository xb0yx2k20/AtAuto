...
def main():
    ...
    for i in range(len(matrix)):
        states.append(str(i))
    lump = Matter()
    machine = Machine(lump, states=states, transitions=trans, initial='0')

    for i in word:
        lump.trigger(i)
    if re.fullmatch(eliminate(alphabet, regex), word) and (lump.state in ends):
        print("Проверка прошла успешно, проверочная строка:", word)
    else:
        print("Проблема")
